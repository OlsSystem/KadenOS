...
module.exports = {
    ...
    data: new SlashCommandBuilder()
        ...
        .setDescription('removes a users points')
        .addUserOption(option => option.setName('user').setDescription('user you are editing').setRequired(true))
        .addIntegerOption(option => option.setName('points').setDescription('how many points u removing').setRequired(true)),
    async execute(interaction) {
            ...
            const logembed = new EmbedBuilder()
                ...
                .setFields(
                    ...
                    { name: "Payout Removed:", value: `${points}` },
                    { name: "New Total of Payout:", value: `${userdata.Points}` },
                    { name: "Issued By", value: `${interaction.user.username}` }
                )
                .setTimestamp()

            await channel.send({ embeds: [logembed] });
            await interaction.followUp({ content: `User ${userdata.UserName} has had ${points} points removed.`, ephemeral: true })
    }
};