...
module.exports = {
    ...
    data: new SlashCommandBuilder()
        .setName('add-certification')
        .setDescription('gives a user a licence')
        .addUserOption(option => option.setName('user').setDescription('user you are editing').setRequired(true))
        .addStringOption(option => option.setName('licence').setDescription('what licence u giving?').setRequired(true)),
    async execute(interaction) {
            ...
            const logembed = new EmbedBuilder()
                .setColor('Red')
                .setTitle('Certification Addition Logs')
                .setFields(
                    { name: "User:", value: `${user} | ${userdata.UserName}` },
                    { name: "Certification Type:", value: `${licence}` },
                    { name: "Issued By", value: `${interaction.user.username}` }
                )
                .setTimestamp()

            await channel.send({ embeds: [logembed] });
            await interaction.followUp({ content: `User ${userdata.UserName} has been awared the ${licence} certification.`, ephemeral: true })
    }
};
